...
// ── Value namespace ──────────────────────────────────────────────────
// Free.api 의 api — 각 명령 이름이 노드 생성 함수. 어휘만 싣는다(예약어 없음) —
// 해석기 문은 Free.interpreter(api, handlers) 로 밖에 있다.
...
export declare const Free: {
    ...
    // Free.interpreter — 한 api 의 해석기. api 는 Free.api(...) 가 만든 것이어야 한다.
    readonly interpreter: <N extends string>(
        api: FreeApi<N>,
        handlers: { [K in N]: (...args: never[]) => unknown }
    ) => FreeApiInterpreter;
