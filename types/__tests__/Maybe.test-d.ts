...
// Helper: assert that T is assignable to U
...
type _1a = Assert<typeof j1, Just<number>>;
type _1b = Assert<typeof n1, Nothing>;
type _1c = Assert<typeof m1, Maybe<number>>;
type _1d = Assert<Just<number>, Maybe<number>>;
type _1e = Assert<Nothing, Maybe<never>>;

// ── 2. map threads the Target slot only ───────────────────────────────
const m2 = Maybe.map((x: number) => String(x), m1);
type _2 = Assert<typeof m2, Maybe<string>>;

// ── 3. Type guards narrow via `this is Just<A>` ───────────────────────
declare const m3: Maybe<string>;
if (m3.isJust()) {
    const v: string = m3.value;  // must narrow to Just<string>
    type _3a = Assert<typeof m3, Just<string>>;
}
if (m3.isNothing()) {
    type _3b = Assert<typeof m3, Nothing>;
}

// ── 4. Functor.of('maybe') resolves via FunctorInstances registry ────
const fMaybe = Functor.of("maybe");
type _4 = Assert<typeof fMaybe, Functor<MaybeTypeLambda>>;
const m4 = fMaybe.map((x: string) => x.length, Maybe.of("hi"));
type _4b = Assert<typeof m4, Maybe<number>>;
...
type _9 = Assert<KMaybe, Maybe<string>>;
...
type _12 = Assert<typeof m12b, Maybe<string>>;
...
type _13 = Assert<typeof m13b, Maybe<number>>;
