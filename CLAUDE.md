...
**`PENDING` 목록** (`tests/docs-examples.test.js`): 아직 수리 중인 문서입니다. 예제를
실행하고 세지만 실패시키지는 않아, 수리하는 동안 게이트가 빨간색이 되지 않습니다.
매 실행마다 남은 개수가 출력되므로 조용히 영구화될 수 없습니다.
**이 목록은 줄기만 합니다** — 비면 목록과 관련 코드를 지우십시오. 새 문서를 여기 추가하지
마십시오.
