...
명세를 따르는 타입 클래스 라이브러리입니다. 의존성 0개, gzip 21KB.
...
| 데이터 타입 | `Maybe` `Either` `Task` `Validation` `Reader` `Writer` `State` `Free` |
| optics | `Lens` `Prism` `Iso` `Traversal` — profunctor 인코딩이라 전부 합성된다 |
| 트랜스포머 | `StateT` `EitherT` `ReaderT` `WriterT` |
| 조합자 | `compose` `pipe` `curry` `flip` `converge` `transducer` … |

ESM 과 CommonJS 둘 다, TypeScript 선언 포함. 문법 상한은 **ES2018** 입니다.

## 문서가 낡지 않습니다

**문서의 예제 417개를 테스트가 실행합니다.** 설명이 코드와 어긋나면 빌드가 멈춥니다.
...
**아직 굳지 않았습니다.** `0.x` 인 이유가 있습니다 — 최근에도 명세 위반을 고치느라 공개
표면이 여러 번 바뀌었고, 그 대부분을 새로 세운 검사 장치가 찾았습니다. 아직 검사가 닿지
않는 곳(`ChainRec`·`Traversable`·`Wander` 의 법칙)이 남아 있어 거기서 더 나올 수 있습니다.
