...
## 닫힘 — Identity 는 클래스이고, 이름은 심볼이 지닌다 (2026-08-15)

- **원인** — `Identity` 를 `{ value, _typeName: 'Identity' }` 객체 리터럴로 만들고 이름만
  넣었다. **이름은 베끼면 된다.** 소유자 판정: *"Object의 하위타입을 만들고 타입이름을
  넣으면서 값싸게 해결하려고 한 것이 잘못된 겁니다."* `_typeName` 의 직무는 합타입 수렴
  (`Just`·`Nothing` 이 둘 다 `'Maybe'`)이지 타입 선언이 아니었다.
- **해결책** — ① `Identity` 를 `Maybe` 와 같은 급의 클래스로(심볼·공개 문·인스턴스 메서드).
  ② `types.of` 가 **심볼만** 보게 한다. `_typeName` 은 눈에 보이는 필드로 남긴다.
- **완료조건** — 이름을 베낀 평범한 객체가 타입 클래스 메서드를 통과하지 못한다.
- **검증** — 태그를 지닌 **열둘 전부**(데이터 타입 9 + `Const`·`Forget`·트랜스포머 4) 위조가
  막힌다. 뮤테이션 8종 전부 잡음(트랜스포머는 넷을 하나씩). `npm test` 45/45 + 타입체크.
  `baseline` 차이 5건 = 위조 4 + `Identity` export 1.
- **비용** — `types.check` 2천만 회 5회 교차 실행: **1.04배**. 먼저 시도한 「이름 대조표」
  방식은 같은 보호에 **2.12배**여서 버렸다.
- **남은 것** — `Const`·`Forget` 은 아직 클래스가 아니다(모노이드마다 태그가 달라 모양이
  다르다). 위조는 이미 막히므로 급하지 않다.
- **참고** — `docs/internals.md#type`, `#identity-const`, `tests/identity.test.js`
...
- **비용** — 약 48곳. `index.js` 3 · 테스트 18 · 문서 23 · `.d.ts` 4.
  AST 작업(227곳)의 5분의 1이라 되돌릴 필요가 없었다.
...
## ⬜ 다음에 볼 것 — `.type` 을 타입 AST 로 (2026-08-14)

**계획서: [`plan/260814-type-ast.md`](./plan/260814-type-ast.md). 착수 전, 결정 넷이 열려 있다.**

> **결정 ④는 위 `plus(...)` 버그 수정으로 절반이 답해졌다.** 출신 기록은 타입이 아니므로
> AST 에 넣지 않는다(소유자 판단). 남은 것은 `const(...)`·`forget(...)`·`statet(...)` 이다 —
> 그쪽은 이름이 `F<X>` 규칙에 맞고 `.type` 이 어긋나는 쪽이라 판정이 다를 수 있다.

- **소유자 지적** — *"`type` 에 array, Set, Map, Triple 등의 컨테이너 타입이 올 때 문제가
  되는 거네요."* 그리고 설계 의도를 밝혔다 — *"타입은 심볼, 값은 문자열. 중첩되면 결국
  AST 가 될 거라 예상했다."*
- **그 예상이 맞았다는 실측** — 지금 조립 키 14종이 **13줄 파서 하나로 전부 파싱된다
  (실패 0).** 문자열 키가 이미 AST 이고, 남은 일은 직렬화된 것을 구조로 올리는 것이다.
- **왜 지금 아픈가** — 등록 인스턴스 133개 중 **82개가 컨테이너**이고 그 태그는 바깥만
  말한다. 이미 터진 사례가 기록돼 있다: `TupleBifunctor` 와 `ArrayFunctor` 가 둘 다
  `.type='Array'` 라 같은 값을 다르게 보는데 게이트가 못 가른다. `Map` 은 키·값 둘이라
  **표현 자체가 안 된다.**
- **크기** — `.type` 자리를 바꾸면 **약 227곳**, 옆에 두면 약 20곳. 다만 옆에 두면 튜플·
  `Map`·`Triple` 이 안 풀린다(그건 `.type` 이 갈라야 한다). 계획서는 227곳을 한 번에
  건드리지 않는 6단계를 제안한다 — 2단계에서 `types.check` 가 문자열과 AST 를 둘 다 받게
  하는 것이 핵심이다.
- **안 건드리는 것** — 심볼 체계 93곳, 값의 표식 `_typeName`. 소유자 설계대로 문자열 쪽만 본다.
- **이 계획이 못 하는 것** — 원소 타입 검사(`map(f, [1,'a'])`)는 저절로 안 생긴다.
  `ArrayFunctor` 는 하나가 모든 배열을 받으므로 인스턴스가 **82개** 더 필요하다.
  AST 는 자리를 만들 뿐이다.
