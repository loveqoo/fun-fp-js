...
## 🟡 진행 중 — index.js 주석 B안: 앵커 없는 다줄 블록 (2026-08-19)
...
## ⏸ 남은 것 — `dist/` 재빌드 (소유자 지시 대기)

오늘 `index.js` 를 여러 회차 고쳤으므로 `dist/` 는 소스와 다르다. `dist-sync` 가 그 이유로
빨강이고, 그것이 지금 유일한 빨강이다. 빌드는 소유자가 지시할 때만 한다(관례).
빌드 순서 규율: **기능 커밋 → 빌드 → dist 커밋**(그래야 헤더 해시가 내용 시점을 가리킨다).
