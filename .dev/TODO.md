...
| 🟡 | 사용-2 | [배포 메타데이터](#사용-2) — LICENSE 만 남음 | 소유자 결정 대기 |
...
<h3 id="사용-2">🟡 [사용-2] 배포 메타데이터 — LICENSE 만 남았다</h3>
...
- **남은 것 — LICENSE.** 소유자 결정이다. 없으면 기본이 "모든 권리 유보" 라 **회사에서 못
  쓴다.** `package.json` 의 `files` 에는 이미 적어 뒀으니 파일만 만들면 된다.
