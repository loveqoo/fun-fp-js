...
## 🟡 진행 중 — 2026-08-14, Optics 를 우리 조각으로 다시 세운다
...
| ⬜ | [2차 — 라이브러리가 안 내주는 세 자리](#optics-2차) | 있음 |
...
<h3 id="optics-2차">⬜ 2차 — 라이브러리가 안 내주는 세 자리</h3>
...
- **참고** — Strong/Choice/Wander 를 타입 클래스로 안 올린 근거는 [`review`·`plan` 기록](./review/)에
  있다. 그 글은 `CLAUDE.md` 에 있었는데 하네스 제거(`b970b96`) 때 함께 지워졌고 `docs/` 로
  옮겨지지 않았다 — **2차를 시작할 때 그 근거를 `docs/internals.md#optics` 로 되살릴지 함께
  정한다.**

---

## ⬜ 발견 — `Const` 는 법칙 게이트가 안 본다 (2026-08-14)
...
<h3 id="사용-2">⬜ [사용-2] 배포 메타데이터가 비어 있다</h3>
