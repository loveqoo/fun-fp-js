...
- **검증** — `npm test` **51/51 + 타입체크**(테스트 파일 50→51), 문서 예제 468개,
  링크 **337개**(229→337).
- **남은 것(2단계 이후)** — 나머지 31개 문서. 큰 것부터: internals(1285줄)·Writer·State·
  Optics·Reader·Actor·Transducer·Free·Functor 계열·트랜스포머 4종. CHANGELOG 는 번역 대상
  밖으로 두었다(변경 기록은 정본 하나가 낫다).
